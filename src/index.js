...
const getDiff = (data1, data2) => {
  ...
  return keys.map((key) => {
    ...
    if (_.isObject(value1) && _.isObject(value2)) {
      return { key, children: getDiff(value1, value2) };
    }

    if (_.has(data1, key) && _.has(data2, key)) {
      return (value1 === value2) ? { key, value: { unchanged: value1 } }
        : { key, value: { removed: value1, added: value2 } };
    }

    return !_.has(data2, key)
      ? { key, value: { removed: value1 } }
      : { key, value: { added: value2 } };
  });
};

const genDiff = (pathToFile1, pathToFile2, format = 'nest') => {
  const diff = getDiff(getParsedData(pathToFile1), getParsedData(pathToFile2));
...
// console.log(getDiff(getParsedData(pathToFile1), getParsedData(pathToFile2))[0]);
